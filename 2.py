...
class AutoLesyaMod(loader.Module):
	...
	async def lesyainfocmd(self, message):
		...
		if stats.get("premium"):
			...
			if stats.get("deluxe"):
			text = text + "🔥 Делюкс бонус: " + timetostr(times.get("deluxe_bonus") - now) + "\n"
	...
	def parsebonus(self, text):
		...
		deluxe = "deluxe" in text
		bonus_type = vip and "vip_bonus" or premium and "premium_bonus" or deluxe and "deluxe_bonus"  or "bonus"
		now = time()
	...
	async def receive(self, message):  # Сообщение от бота
		...
		if settings.get("opencase"):
			...
			elif ", ващи кейсы" in text: # Закончился этот кейс
		...
		while True:
			...
			if settings.get("bonus") and not sleep:
				...
				if stats.get("premium") and now > times.get("premium_money"):
					...
					if stats.get("deluxe") and now > times.get("premium_bonus"):
					logger.info("Getting premium bonus")
					times["deluxe_bonus"] = now + 600
					asyncio.ensure_future(self.send_bot("Делюкс бонус"))
